...
uniform sampler2D u_noise;   // 噪声贴图

uniform float u_time;
uniform vec2 u_texsize;      // 纹理尺寸（像素）
...
void main() {
    ...
    float rgbIntesnsity      = 0.1 + 0.1 * sin(u_time * 3.7);
    float displaceIntesnsity = 0.2 + 0.3 * pow(sin(u_time * 1.2), 5.0);
