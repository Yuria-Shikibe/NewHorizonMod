...
public class UpgradeAmmoData extends UpgradeMultData{
	...
	public void buildUpgradeInfoAll(Table t2) {
		t2.table(Tex.button, t -> {
			t.pane(table -> table.image(icon).size(LEN).left()).size(LEN).left();
		...
		new Dialog("") {{
			...
			cont.pane(table -> {
				...
				for(Field field : fields){
					try{
						if(field.getGenericType().toString().equals("java.lang.Boolean"))table.add(new StringBuilder().append("[gray]").append(field.getName()).append(": ").append(getJudge(field.getBoolean(selectAmmo))).append("[]")).left().row();
						if(field.getGenericType().toString().equals("java.lang.Float"))table.add(new StringBuilder().append("[gray]").append(field.getName()).append(": [lightgray]").append(field.getFloat(selectAmmo)).append("[]")).left().row();
						if(field.getGenericType().toString().equals("java.lang.Integer"))table.add(new StringBuilder().append("[gray]").append(field.getName()).append(": [lightgray]").append(field.getInt(selectAmmo)).append("[]")).left().row();
