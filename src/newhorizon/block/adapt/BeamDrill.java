...
public class BeamDrill extends Drill{
	...
	public class BeamDrillBuild extends DrillBuild{
		...
		public void updateTile(){
			if(!(items.total() < itemCapacity && dominantItems > 0 && consValid()))warmup = Mathf.lerpDelta(warmup, 0, coolSpeed);
			super.updateTile();
		}
		...
		public void draw(){
			...
			float stroke = laserScl * Mathf.curve(warmup, 0, (items.total() < itemCapacity && dominantItems > 0 && consValid()) ? efficiency() : 1);
			Draw.mixcol(laserColor, Mathf.absin(4f, 0.6f));
			Draw.alpha(laserAlpha + Mathf.absin(8f, laserAlphaSine));
			Draw.blend(Blending.additive);
			Drawf.laser(team, laser, laserEnd, x + (-shooterOffset + warmup * shooterExtendOffset + shootY), moveY, x - (-shooterOffset + warmup * shooterExtendOffset + shootY), moveY, stroke);
			Drawf.laser(team, laser, laserEnd, moveX, y + (-shooterOffset + warmup * shooterExtendOffset + shootY), moveX, y - (-shooterOffset + warmup * shooterExtendOffset + shootY), stroke);
