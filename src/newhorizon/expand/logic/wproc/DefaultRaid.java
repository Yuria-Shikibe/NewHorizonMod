...
public class DefaultRaid extends LStatement {
    ...
    public static void showRaidAlertHud(String timerName, float time, float range, float sx, float sy, float tx, float ty) {
    ...
    public class DefaultRaidInstruction implements LExecutor.LInstruction {
        ...
        public void showLabel() {
            NHUIFunc.showToast(NHContent.raid, "[#ff7b69]Raid: []<" + (int) (target.x / tilesize) + "," + (int) (target.y / tilesize) + ">",
        ...
        public void showAlert() {
            ...
            if (!headless) {
                showRaidAlertHud(timer.name, alertTime.numf(), inaccuracy.numf(), source.x, source.y, target.x, target.y);
            } else {
                state.rules.objectives.each(mapObjective -> {
